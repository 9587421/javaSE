...
在面向过程的语言中，完成某件事的代码块被称为函数

在面向对象的语言中，完成某件事情的代码块被称为方法

##### 1.什么是方法？

​	方法就是一段有名称的代码块，达到代码复用的作用，例如我们之前做输入时使用的Scanner，简单来说Scanner就是一个写好的代码块，当我们需要的时候，把他通过对象直接进行使用，这是代码复用。

（姑且先这么去想像，事实上这么理解是有问题的，在此只是为了做一个代码复用，方法是什么的导入，不过学到后面自然就明白了。）

使用方法的好处：代码复用，一次编写，到处使用。

##### 2.方法的结构

修饰符 特殊关键字 返回值类型 方法名字（参数列表）{
...
​	【说明】特殊关键字，可以没有，也可以同时有 

​	【说明】修饰符 和 特殊关键字，是可以调换位置的     	
...
​	void：方法没有返回值，无法获取方法的处理结果

​	基本、应用类型：有返回值

方法名：驼峰命名法，在使用方法时，就通过方法名来使用方法

形参：就是在方法的()里声明的变量，但是声明的同时不能赋值，可以没有，也可以有很多个参数，用于接收方法的调用者，传递的值



方法体：方法中实现某个功能的代码块

​		方法体中可以使用方法名后（）内定义的参数变量

return：把处理到最后的结果，传递给方法的使用处。

##### 方法分类与使用

方法可以分为三类，有参有返，有参无返，无参无返。
...
```java
//声明方法，【注意】方法的声明位置：在类内部，在其他方法外部
    //声明无返回值、无参数的方法
    //void:无返回值，在方法体中，不能返回值，不能使用return关键，是矛盾体，不能同时出现
    //() 内没有任何变量，称为无参数
    public static void demo(){
        //计算两个数的最大公约数
        int num1=12;
        int num2=18;
        //最大公约数：同时能把这两个数除开的最大的数
        //1、先找到两个数中较小的数
...
1、方法声明后，并不会执行，只有调用后，才会被执行

 方法的调用： 方法名(参数);                      

​	调用一个方法，就是执行这个方法里的代码      

​	main方法是程序执行的入口，所以，在main方法里调用
...
​	无参数的方法，只能计算固定的两个数的最小公约数，比较单一，使用参数来接收要计算的两个数， 比较灵活。

public   static   void  com2(int num1,int num2){

}

参方法，接收两个数，方法体，计算这两个数的最小公倍数练习：
...
```java
 public class Demo4 {
    public static void main(String[] args) {
        ...
        //12、18 是具体的值，在调用方法时使用的值、变量，称为实际参数，简称实参
        com2(12,18);
        //方式2：调用时使用变量
        //实参和形参可以同名，也可以不同名
        int num1=12;
        ...
        com2(n1,n2);
        //【注意】实参和形参是一一对应的：类型、个数、顺序都要一一对应
        System.out.println("第四次调用");
        com2(n2,n1);
    }
    //编写方法，计算两个数的最小公倍数
    //num1、num2分别用于接收要计算的两个数
    //在声明方法时，定义的参数，称为形式参数，简称形参
    //参数可以是0到若干个，之间使用逗号分隔
    public static void com2(int num1, int num2){
        //在方法体里，可以直接使用形参
        //1、找出两个数中较大的数
        ...
        while(true){
            if(sum%num1==0 && sum%num2==0){
                System.out.println(num1+"和"+num2+"的最小公倍数："+sum);
                break;
            }
            sum+=max;
        }
